...
class Voxel_loader(object):
	def __init__(self, data_location,  set_type='train'):

		self.data_location = data_location
		print (f'The number of {set_type} set objects found : {len(self.data_location)}')

	def __len__(self): 
		return len(self.data_location)

	def __getitem__(self, index): 
		data = {}

		data['id'] = self.data_location[index]
		
		#load voxels 
		voxels = sio.loadmat(self.data_location[index]+'/model')['model']
		data['voxels'] = torch.FloatTensor(voxels)

		#load mesh 
		verts, faces, aux = load_obj(self.data_location[index]+'/model.obj')
	...
	def collate(self, batch):
		data = {}
		data['id'] = [item['id']for item in batch]
		...
		data['edges'] = [item['edges']for item in batch]
		data['adjs'] = [item['adj'] for item in batch]

		return data


class GCN_Loader(object):
	def __init__(self, data_location,  set_type='train'):
