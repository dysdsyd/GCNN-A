...
class Pixel2MeshHead(nn.Module):
    def __init__(self, cfg):
        ...
        cfg.MODEL.MESH_HEAD.COMPUTED_INPUT_CHANNELS = sum(feat_dims)
        self.mesh_head = MeshRefinementHead(cfg)
        self.init_meshes = ico_sphere(self.ico_sphere_level)
    ...
    def forward(self, imgs):
        ...
        self.init_meshes = self.init_meshes.extend(N).to(device)
        refined_meshes = self.mesh_head(img_feats, self.init_meshes, P, subdivide=True)
        return None, refined_meshes
