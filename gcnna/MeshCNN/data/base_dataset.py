...
def collate_fn(batch):
    ...
    meta['label'] = torch.Tensor(meta['label']).long()
    meta['features'] = torch.cat(meta['features'], dim=0)
    try:
         meta['edges'] = torch.cat(meta['edges'], dim=0)
    except:
        pass
    try:
         meta['adj'] = torch.cat(meta['adj'], dim=0)
    except:
        pass
    return meta