...
from .layers.mesh_conv import MeshConv, ZERON_GCN
...
def define_classifier(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain):
    ...
    if arch == 'mconvnet':
        net = MeshConvNet(opt.method, norm_layer, input_nc, ncf, nclasses, ninput_edges, opt.pool_res, opt.fc_n,
                          opt.resblocks, opt)
    elif arch == 'meshunet':
...
class MeshConvNet(nn.Module):
    ...
    """
    def __init__(self, method, norm_layer, nf0, conv_res, nclasses, input_res, pool_res, fc_n,
                 nresblocks=3, opt=None):
        ...
        for i, ki in enumerate(self.k[:-1]):
            if method == 'edge_cnn':
                ...
                setattr(self, 'conv{}'.format(i), MResConvGCN(ki, self.k[i + 1], nresblocks))
            elif method == 'zgcn_cnn':
                setattr(self, 'conv{}'.format(i), MResConvZGCN(ki, self.k[i + 1], nresblocks))
            setattr(self, 'norm{}'.format(i), norm_layer(**norm_args[i]))
            # setattr(self, 'pool{}'.format(i), MeshPool(self.res[i + 1]))
 
        self.gp = torch.nn.AvgPool1d(self.res[0])
        # self.gp = torch.nn.MaxPool1d(self.res[0])
        self.fc1 = nn.Linear(self.k[-1], fc_n)
        self.fc2 = nn.Linear(fc_n, nclasses)

    def extract_feats(self, x, mesh, layer, verbose = False):
        '''
        layer = res_k_conv
        '''
        ...
        for i in range(len(self.k) - 1):
            x1, x2, x = getattr(self, 'conv{}'.format(i))(x, mesh, True)
            if layer == 'res_'+str(i)+'_conv0':
                return x1.squeeze(-1).permute(0,2,1)
            elif layer == 'res_'+str(i)+'_conv1':
                return x2.squeeze(-1).permute(0,2,1)
            elif layer == 'res_'+str(i)+'_conv_add':
                return x.squeeze(-1).permute(0,2,1)
            if verbose: 
                print('res_'+str(i)+'_conv0: ', x1.squeeze(-1).permute(0,2,1).shape)
                print('res_'+str(i)+'_conv1: ', x2.squeeze(-1).permute(0,2,1).shape)
                print('res_'+str(i)+'_conv_add: ', x.squeeze(-1).permute(0,2,1).shape)
            x = F.relu(getattr(self, 'norm{}'.format(i))(x))
            # x = getattr(self, 'pool{}'.format(i))(x, mesh)
            # if layer == 'res_'+str(i)+'_pool':
            #     return x.squeeze(-1).permute(0,2,1)
            # if verbose: print('res_'+str(i)+'_pool: ', x.squeeze(-1).permute(0,2,1).shape)

        x = self.gp(x.squeeze(-1))
        if verbose: print('GP: ', x.shape )
        x = x.view(-1, self.k[-1])
        if verbose: print('GP Reshape: ', x.shape)
        if layer == 'gp':
                return x
        ...
        x = self.fc2(x)
        if verbose: print('fc2: ', x.shape)
        return x

    def forward(self, x, mesh, verbose=False):
        # In case of graph CNNs mesh is edges or adjacency
        if self.opt.method != 'edge_cnn':
            x = x.permute(0,2,1).unsqueeze(-1) # B * F * V * 1 
        if verbose: print('Input: ', x.shape)
        for i in range(len(self.k) - 1):
            x = getattr(self, 'conv{}'.format(i))(x, mesh)
            if verbose: print('res_'+str(i)+'_conv: ', x.shape)
            x = F.relu(getattr(self, 'norm{}'.format(i))(x))
            if verbose: print('res_'+str(i)+'_bn: ', x.shape)
            # x = getattr(self, 'pool{}'.format(i))(x, mesh)
        
        if verbose: print('before gp: ', x.shape )
        x = self.gp(x.squeeze(-1))
        if verbose: print('after gp: ', x.shape )
        x = x.view(-1, self.k[-1])
        if verbose: print('reshape gp: ', x.shape )
        x = F.relu(self.fc1(x))
        if verbose: print('fc1: ', x.shape )
        x = self.fc2(x)
        if verbose: print('fc2: ', x.shape )
...
class MResConv(nn.Module):
    ...
    def forward(self, x, mesh, return_all = False):
        x = self.conv0(x, mesh)
        x1 = x
        for i in range(self.skips):
...
class MResConvGCN(nn.Module):
    def __init__(self, in_channels, out_channels, skips=1):
        super(MResConvGCN, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.skips = skips
        self.conv0 = GraphConv(self.in_channels, self.out_channels)
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm2d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
                    GraphConv(self.out_channels, self.out_channels))

    def graph_conv(self, x, edges, conv):
        '''
        x: B * F * V * 1
        edges: B * E * 3
        ...
        returns:
            B * F * V * 1 
        '''
        assert x.shape[0] == edges.shape[0]
        B = x.shape[0]
        x = x.squeeze(-1) # B*F*V
        out = []
        for b in range(B):
            v = x[b].permute(1,0) # V*F
            e = edges[b]
            out.append(conv(v, e).permute(1,0).unsqueeze(0)) # 1 * F' * V 
        return torch.cat(out, dim=0).unsqueeze(-1) # B * F' * V * 1
            


    def forward(self, x, edges, return_all = False):
        x = self.graph_conv(x, edges, self.conv0)
        x1 = x
        for i in range(self.skips):
            x = getattr(self, 'bn{}'.format(i + 1))(F.relu(x))
            x = self.graph_conv(x, edges, getattr(self, 'conv{}'.format(i + 1)))
        x2 = x
        x += x1
        x = F.relu(x)
        if return_all:
...
class MResConvZGCN(nn.Module):
    def __init__(self, in_channels, out_channels, skips=1):
        super(MResConvZGCN, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.skips = skips
        self.conv0 = ZERON_GCN(self.in_channels, self.out_channels)
        for i in range(self.skips):
            setattr(self, 'bn{}'.format(i + 1), nn.BatchNorm2d(self.out_channels))
            setattr(self, 'conv{}'.format(i + 1),
                    ZERON_GCN(self.out_channels, self.out_channels))

    def graph_conv(self, x, adj, conv):
        '''
        x: B * F * V * 1
        edges: B * E * 3
        ...
        for b in range(B):
            v = x[b].permute(1,0) # V*F
            a = adj[b]
            out.append(conv(v, a).permute(1,0).unsqueeze(0)) # 1 * F' * V 
        return torch.cat(out, dim=0).unsqueeze(-1) # B * F' * V * 1
            


    def forward(self, x, adj, return_all = False):
        x = self.graph_conv(x, adj, self.conv0)
        x1 = x
        for i in range(self.skips):
            x = getattr(self, 'bn{}'.format(i + 1))(F.relu(x))
            x = self.graph_conv(x, adj, getattr(self, 'conv{}'.format(i + 1)))
        x2 = x
        x += x1
        x = F.relu(x)
        if return_all:
