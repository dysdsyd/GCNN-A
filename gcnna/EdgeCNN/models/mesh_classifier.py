...
class ClassifierModel:
    ...
    def set_input(self, data):
        self.features = data['features'].to(self.device)#.requires_grad_(self.is_train)
        if self.opt.method == 'gcn_cnn':
            self.edges = data['edges'].to(self.device)
    ...
    def forward(self):
        if self.opt.method == 'edge_cnn':
            ...
            # for graph based cnns use edges
            out = self.net(self.features, self.edges)
        elif self.opt.method == 'zgcn_cnn':
            # for graph based cnns use adj
            out = self.net(self.features, self.adj)
        return out
