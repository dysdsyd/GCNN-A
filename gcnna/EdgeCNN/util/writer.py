...
class Writer:
    def __init__(self, opt):
        ...
        if opt.is_train and not opt.no_vis and SummaryWriter is not None:
            self.display = SummaryWriter(os.path.join('../../tensorboard/',opt.name))
        else:
            self.display = None
    ...
    def plot_loss(self, loss, epoch, i, n):
        iters = i + (epoch - 1) * n
        if self.display:
            self.display.add_scalar(self.name+'/train_loss', loss, iters)
    ...
    def plot_acc(self, acc, epoch):
        if self.display:
            self.display.add_scalar(self.name+'/test_acc', acc, epoch)
