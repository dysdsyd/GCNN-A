...
def pad(input_arr, target_length, val=0, dim=1, mode='constant', method='edge_cnn'):
    shp = input_arr.shape
    # npad = [(0, 0) for _ in range(len(shp))]
    # print(npad)
    if method =='edge_cnn':
        npad = (0, target_length - shp[dim])
    else:
        npad = (0, 0, 0, target_length - shp[dim])
    return torch.nn.functional.pad(input_arr, pad=npad, mode=mode, value=val)
